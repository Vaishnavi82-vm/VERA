...
import { listContactMessages, sendContactMessage, deleteContactMessage, type CloudContactMessage } from '@/lib/cloud';
...
export default function Contact() {
  ...
  const [history, setHistory] = useState<CloudContactMessage[]>([]);

  useEffect(() => {
    if (email) {
      listContactMessages(email).then(setHistory);
    }
  }, [email]);
            ...
            {history.map(m => (
              <li key={m.id} className="bg-cream rounded-2xl border border-border/40 shadow-card p-4 animate-fade-in">
                <div className="flex items-center justify-between mb-1.5">
                  ...
                  <span className="font-body text-[10px] text-muted-foreground tracking-wider">{formatDate(m.created_at)}</span>
                </div>
