...
import { listWardrobe, getPreferences, type CloudPreferences, type CloudWardrobeItem, listEvents, addEvent, type CloudEvent } from '@/lib/cloud';
...
export default function HomePage() {
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    Promise.all([getPreferences(email), listWardrobe(email), listEvents(email)])
      .then(([loadedPrefs, loadedWardrobe, loadedEvents]) => {
        if (cancelled) return;
      ...
      .catch(() => {
        // Ignore load failures; page should still render.
      });
  ...
  const wardrobeItems = useMemo(
    () => wardrobe.map((item) => ({
      ...
      type: item.category || item.title || item.name,
      color: item.primary_color || item.description || 'neutral',
  ...
  const outfits = useMemo(
    () => generateOutfits(wardrobeItems, weather, prefs as unknown as any, undefined, 7),
    [wardrobeItems, weather, prefs]
  );
  ...
  const handleAddEvent = () => {
    if (selectedDate && eventName) {
      addEvent(email, { date: selectedDate, event: eventName, location: eventLocation })
        .then(newEvent => {
            ...
            {(() => {
              ...
              return event ? (
                <div className="mb-3 p-2.5 bg-nude-soft rounded-xl border border-nude/40">
                  <p className="font-body text-xs text-foreground">📌 {event.event}</p>
                </div>
              ) : null;
            })()}
