...
export default function OutfitsPage() {
  ...
  const load = async () => {
    ...
    try {
      const [w, o] = await Promise.all([
        ...
        listOutfits(user.email, { limit: 12 }),
      ]);
      setWardrobe(w);
      setOutfits(o);
    } catch (e) {
  ...
  const regenerate = async () => {
    ...
    setGenerating(true);
    try {
      const fresh = await generateOutfits(user.email, { occasion, mood, count: 5 });
      if (!fresh.length) {
  ...
  const onWear = async (o: CloudOutfit) => {
    ...
    if (user) await recordFeedback(user.email, o.id, { liked: true, worn: true });
    setOutfits((p) => p.map((x) => x.id === o.id ? { ...x, worn: true, liked: true } : x));
    toast.success('Marked as worn today');
  };

  const onSkip = async (o: CloudOutfit) => {
    if (user) await recordFeedback(user.email, o.id, { disliked: true });
      ...
      <div className="p-5">
        <div className="flex items-start justify-between gap-3 mb-2">
          <div>
            ...
            <h3 className="font-display text-xl italic text-foreground leading-tight">{outfit.outfit_name}</h3>
          </div>
        </div>
