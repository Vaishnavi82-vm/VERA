...
import { getPreferences, savePreferences, type CloudPreferences } from '@/lib/cloud';
  ...
  useEffect(() => {
    ...
    getPreferences(email)
      .then((prefs) => {
        ...
        setLifestyle(prefs.lifestyle || 'corporate');
        setStyle(prefs.style || 'elegant');
        setSleevelessAllowed(prefs.restrictions?.sleevelessAllowed ?? true);
        setShortAllowed(prefs.restrictions?.shortOutfitsAllowed ?? true);
      })
      .catch(() => {
  ...
  const handleSave = async () => {
    ...
    try {
      await savePreferences(email, {
        ...
        restrictions: {
          sleevelessAllowed,
          shortOutfitsAllowed: shortAllowed,
        },
      });
      setSaved(true);
