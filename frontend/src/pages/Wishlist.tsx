...
import { listWardrobe, getWishlist, toggleWishlistItem, type CloudWardrobeItem } from '@/lib/cloud';
...
export default function Wishlist() {
  ...
  useEffect(() => {
    if (email) {
      Promise.all([listWardrobe(email), getWishlist(email)])
        .then(([items, ids]) => {
          setWardrobe(items);
          setWishlist(ids);
        });
